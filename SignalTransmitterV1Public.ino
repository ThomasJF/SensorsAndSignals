...
/*
              **************************************************
              **    WiFI: Set SSID and Password on line 95    **
              **************************************************
Trace:        Set Trace Output 0 (off) 1 (on) on line 92. If Sound output is bound to one of the sensors trace is set to 0.
Interfaces:
...
Signal:       either 4 pin plug with +3.3 and 3 Bits ord solder points witg 3.3v and 4 Bits. Acive low. External resistors.
              ...
              Signal Mapping on line 103. Ports are active low, a zero bit means LED on. A value of 0x0f (15) means all LEDs off.
              The mapping defines what LEDs are lit for which aspect-code comin from Rocrail            
...
Factory reset: Sets the WLAN to DHCP and some of the parameters to a default value. Use the DDHCP-Server's DB to find the adddress and
              connect with the browser to this IP. 
              If you pull the LED-Pin on the side towards the ESP-03 to ground (testpoint) for 2 or 3 seconds during power up, the board will
              reboot in factory reset mode.
...
Webinterface: IP-Address-Change requires reboot. Other parameters are applied on the fly.
              Server IP
              Some parameters are only reconfigurable via source code, such as UID/Pass for WLAN, Port for UDP (line 156), trace on/off, etc.
...
OP-Codes:     Send:
              ...
              PLAYS,  CH,   SoundNr     Play Sound (see MP3 below)
              BOOT!,  n/a,  n/a         REBOOT 
...
//
// configure wifi
const char* ssid = "WiFi SSID";        //  your network SSID (name)
const char* pass = "WiFi Password";      // your network password
//
Ticker ticker10ms;
...
// EEPROM or Default Values    
    ...
    if(factory){
        ...
        ACK_Timeout=0;
        EEPROM.put(8,ACK_Timeout);
        power[0]=15,power[1]=15,play[0]=255,play[1]=255;
        EEPROM.put(12,power[0]),EEPROM.put(13,power[1]);
        EEPROM.put(14,play[0]),EEPROM.put(15,play[1]);   
